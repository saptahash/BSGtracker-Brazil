...
city_gov <- read_feather("./brazil_citygov_risk_of_openness.feather")
...
plot <-
  ...
  #lims(colour = c("0", "1")) + 
  geom_text_repel(data = subset(base_tibble, (mean_stringency < 45 & mean_rooi > 0.6)),
                  size = 2, colour = "black") +
...
ggsave(plot = plot,
       filename = "./brazilrooi_citygov_scatterplotdetail_monthly_popul.png", 
       width = 12, 
       height =8)
...
temp_tibble <- 
  unique(
  city_gov %>%
  select(CityName, RegionName)
  )

write.csv(temp_tibble, "./brazil_citystateregions.csv")
